...
WS: `/ws/terminal/:runId?token=…` (history/data/exit ↔ input/resize, base64-in-JSON) and `/ws/events` (task.updated, run.started/exited, run.needs-attention, run.activity, proposal.created, feature.updated/deleted, orchestrator.status) — frontend fully event-driven, no polling.
